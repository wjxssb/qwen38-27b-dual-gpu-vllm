...
from vllm.v1.executor.tp_lifecycle import GenerationLifecycleError
...
# Later client ADDs wait only while the single active lifecycle generation
# reaches its ordinary worker-cleanup boundary.  Keeping this host-side queue
# finite prevents a stalled or poisoned TP group from retaining an unbounded
# number of request payloads.
_STABILITY_DEFERRED_ADD_CAPACITY = 256
...
class EngineCore:
    ...
    def __init__(
        ...
        # Request completion is deliberately finalized only after a subsequent
        # normal SchedulerOutput has delivered ``finished_req_ids`` to every
        # worker.  This set holds those final outputs until that real cleanup
        # path has run; it is not a parallel cache or a force-reset mechanism.
        self._stability_finished_request_ids_pending_cleanup: set[str] = set()
        # Only the isolated generation-lifecycle executor uses this queue.
        # ADD messages for a later request wait here while the active
        # generation reaches its normal worker ``finished_req_ids`` cleanup
        # boundary.  They are not re-enqueued, force-finished, or scheduled
        # before the prior TP-wide baseline has passed.
        self._stability_deferred_adds: deque[tuple[Request, int]] = deque()
        self._stability_deferred_add_capacity = _STABILITY_DEFERRED_ADD_CAPACITY
    ...
    def add_request(self, request: Request, request_wave: int = 0):
        ...
        # An ADD for an existing scheduler request is a streaming-input update,
        # not a new distributed request.  Preserve vLLM's public streaming
        # semantics by keeping it inside the owning active generation.
        existing_scheduler_request = request.request_id in getattr(
            self.scheduler, "requests", {}
        )
        lifecycle_begin = getattr(
            self.model_executor, "stability_begin_generation", None
        )
        lifecycle_started = False
        if callable(lifecycle_begin) and not existing_scheduler_request:
            lifecycle_begin(request.request_id)
            lifecycle_started = True
        try:
            self.scheduler.add_request(request)
        except Exception:
            # There is no later ``finished_req_ids`` worker-cleanup boundary
            # for a scheduler admission failure.  A lifecycle-aware executor
            # must poison this generation now; feature-off and legacy test
            # executors retain the prior abort callback.
            lifecycle_admission_failed = getattr(
                self.model_executor, "stability_admission_failed", None
            )
            if lifecycle_started and callable(lifecycle_admission_failed):
                lifecycle_admission_failed(request.request_id)
            elif lifecycle_started:
                lifecycle_abort = getattr(
                    self.model_executor, "stability_mark_abort", None
                )
                if callable(lifecycle_abort):
                    lifecycle_abort(request.request_id)
            raise
    ...
    def abort_requests(self, request_ids: list[str]):
        ...
        lifecycle_abort = getattr(self.model_executor, "stability_mark_abort", None)
        if callable(lifecycle_abort):
            for request_id in request_ids:
                lifecycle_abort(request_id)
    ...
    def step(self) -> tuple[dict[int, EngineCoreOutputs], bool]:
        ...
        with (
            ...
            if model_output is None:
                self._record_stability_sample_context(scheduler_output)
        ...
        self._stability_mark_finished_request_outputs(engine_core_outputs)
        self._stability_finalize_worker_cleanups(scheduler_output)
        self._attach_iteration_details(engine_core_outputs, iteration_details)
        self._emit_stability_scheduler_boundary(scheduler_output, engine_core_outputs)
    ...
    def _stability_mark_finished_request_outputs(
        self, engine_core_outputs: dict[int, EngineCoreOutputs]
    ) -> None:
        """Record final request outputs before worker-side cleanup is observed."""
        marker = getattr(self.model_executor, "stability_mark_completing", None)
        if not callable(marker):
            return
        finished_request_ids = {
            request_id
            for output in engine_core_outputs.values()
            for request_id in (output.finished_requests or ())
        }
        for request_id in sorted(finished_request_ids):
            marker(request_id)
            self._stability_finished_request_ids_pending_cleanup.add(request_id)

    def _stability_finalize_worker_cleanups(
        self, scheduler_output: SchedulerOutput
    ) -> None:
        """Commit a generation only after normal ``finished_req_ids`` cleanup.

        The Worker/GPUModelRunner owns request-state removal.  This method only
        observes the post-cleanup boundary and asks the TP lifecycle to prove
        quiescence; it never edits worker, MTP, GDN, KV, or CUDA state.
        """
        finalizer = getattr(self.model_executor, "stability_finalize_generation", None)
        if not callable(finalizer):
            return
        cleaned_request_ids = set(scheduler_output.finished_req_ids)
        # ``EngineCoreOutputs.finished_requests`` is intentionally optional
        # (normally enabled only for internal DP balancing).  The scheduler's
        # next real worker cleanup output is the authoritative completion
        # boundary in every mode, so discover an otherwise-unreported terminal
        # request here rather than depending on that optional client-facing
        # field.
        marker = getattr(self.model_executor, "stability_mark_completing", None)
        if callable(marker):
            for request_id in sorted(
                cleaned_request_ids
                - self._stability_finished_request_ids_pending_cleanup
            ):
                marker(request_id)
                self._stability_finished_request_ids_pending_cleanup.add(request_id)
        for request_id in sorted(
            cleaned_request_ids & self._stability_finished_request_ids_pending_cleanup
        ):
            finalizer(
                request_id,
                engine_active_request_count=len(self.scheduler.requests),
                engine_active_request_ids=sorted(self.scheduler.requests),
            )
            self._stability_finished_request_ids_pending_cleanup.discard(request_id)

    def _emit_stability_scheduler_boundary(
        self,
        ...
        try:
            ...
            if not finished_request_ids and self.scheduler.has_requests():
                return

            kv_cache_manager = self.scheduler.kv_cache_manager
            block_pool = kv_cache_manager.block_pool
            executor_snapshotter = getattr(
                self.model_executor, "stability_diagnostic_snapshot", None
            )
            executor = executor_snapshotter() if callable(executor_snapshotter) else {}
            mamba_managers: list[dict[str, Any]] = []
            for manager in kv_cache_manager.coordinator.single_type_managers:
                last_state_block_idx = getattr(manager, "last_state_block_idx", None)
                allocated_block_reqs = getattr(manager, "_allocated_block_reqs", None)
                if last_state_block_idx is not None or allocated_block_reqs is not None:
                    mamba_managers.append(
                        {
                            "manager_type": type(manager).__name__,
                            "last_state_block_idx_count": len(
                                last_state_block_idx or {}
                            ),
                            "allocated_block_request_count": len(
                                allocated_block_reqs or ()
                            ),
                        }
                    )

            emit_stability_telemetry(
                "scheduler_request_boundary",
                finished_request_ids=finished_request_ids,
                scheduled_finished_request_ids=sorted(
                    scheduler_output.finished_req_ids
                ),
                active_request_ids=sorted(self.scheduler.requests),
                active_request_count=len(self.scheduler.requests),
                running_request_count=len(self.scheduler.running),
                waiting_request_count=len(self.scheduler.waiting),
                batch_queue_depth=(
                    len(self.batch_queue) if self.batch_queue is not None else 0
                ),
                kv_cache={
                    "usage": kv_cache_manager.usage,
                    "free_blocks": block_pool.get_num_free_blocks(),
                    "total_blocks": block_pool.num_gpu_blocks,
                    "deferred_free_count": len(self.scheduler.deferred_frees),
                    "scheduled_step_seq": self.scheduler.sched_step_seq,
                    "processed_step_seq": self.scheduler.processed_step_seq,
                },
                mamba_managers=mamba_managers,
                executor=executor,
            )
        except Exception:
            return

    def _record_stability_sample_context(
        self, scheduler_output: SchedulerOutput
    ) -> None:
        progress_recorder = getattr(
            self.model_executor, "stability_note_scheduler_progress", None
        )
        if not stability_telemetry_enabled() and not callable(progress_recorder):
            return
        recorder = getattr(self.model_executor, "record_stability_sample_context", None)
        ...
        if callable(progress_recorder):
            progress_recorder(scheduler_output)
    ...
    def step_with_batch_queue(
        ...
        if deferred_scheduler_output:
            ...
            self._record_stability_sample_context(deferred_scheduler_output)
...
class EngineCoreProc(EngineCore):
    ...
    def _process_input_queue(self):
        ...
        self._stability_admit_one_deferred_add()
    ...
    def _process_engine_step(self) -> bool:
        ...
        try:
            ...
            self._handle_tp_lifecycle_failure(exc)
            return False
        ...
        try:
            ...
            # MTP draft-token collection is a request-scoped collective.  If
            # it loses its lifecycle proof, use the same fail-closed boundary
            # as execute_model rather than letting the EngineCore busy loop
            # escape without the poisoned-group notification.
            self._handle_tp_lifecycle_failure(exc)
            return False
    ...
    def _handle_tp_lifecycle_failure(self, exc: GenerationLifecycleError) -> None:
        """Fail the affected work but leave a poisoned TP group unavailable.

        The source process must not try to locally reset a worker, force-free
        SHM, or reuse model state after a failed quiescence proof.  The bounded
        external stability supervisor observes the poison status and replaces
        the whole EngineCore/TP worker instance from the immutable profile.
        """
        logger.error("TP lifecycle entered fail-closed state: %s", exc)
        failed_requests = self.scheduler.finish_requests(
            None, RequestStatus.FINISHED_ABORTED
        )
        self._stability_finished_request_ids_pending_cleanup.clear()
        deferred_request_ids = self._stability_flush_deferred_adds(aborted=False)
        if failed_requests:
            by_client: dict[int, list[str]] = defaultdict(list)
            for request in failed_requests:
                by_client[request.client_index].append(request.request_id)
            for client_index, request_ids in by_client.items():
                self._send_error_outputs_to_client(request_ids, client_index)
        emit_stability_telemetry(
            "tp_lifecycle_request_failure",
            exception_type=type(exc).__name__,
            message=str(exc),
            failed_request_ids=(
                [request.request_id for request in failed_requests]
    ...
    def _handle_shutdown(self) -> bool:
        ...
        if self.shutdown_state == EngineShutdownState.REQUESTED:
            ...
            # Deferred ADDs never reached scheduler or worker ownership.  A
            # shutdown cannot leave them invisible behind a poisoned or
            # draining generation, so return their normal abort response now.
            self._stability_flush_deferred_adds(aborted=True)
    ...
    def _handle_client_request(
        ...
        if request_type == EngineCoreRequestType.WAKEUP:
            ...
            if self._reject_add_in_shutdown(req):
                return
            admission_state = self._stability_request_admission_state()
            existing_scheduler_request = req.request_id in getattr(
                self.scheduler, "requests", {}
            )
            if admission_state == "busy" and not existing_scheduler_request:
                # The request never entered scheduler/worker ownership.  It
                # can therefore preserve immediate-abort semantics without
                # waiting behind a long active generation.
                if req.abort_immediately:
                    self._send_abort_outputs_to_client(
                        [req.request_id], req.client_index
                    )
                    return
                if len(self._stability_deferred_adds) >= getattr(
                    self,
                    "_stability_deferred_add_capacity",
                    _STABILITY_DEFERRED_ADD_CAPACITY,
                ):
                    logger.warning(
                        "TP lifecycle deferred-ADD queue is full; rejecting "
                        "request_id=%s",
                        req.request_id,
                    )
                    ...
                    return
                self._stability_deferred_adds.append((req, request_wave))
                return
            if admission_state == "poisoned":
                self._send_error_outputs_to_client([req.request_id], req.client_index)
                return
            try:
                self.add_request(req, request_wave)
            except GenerationLifecycleError as exc:
                # A poisoned TP group is intentionally unavailable until the
                # external full-instance supervisor proves a fresh TP2 group.
                # Reject this request without crashing the EngineCore or
                # admitting it behind an unresolved generation.
                logger.error(
                    "TP lifecycle rejected request_id=%s: %s", req.request_id, exc
                )
                self._send_error_outputs_to_client([req.request_id], req.client_index)
        elif request_type == EngineCoreRequestType.ABORT:
            deferred_ids = self._stability_abort_deferred_requests(request)
            active_request_ids = set(
                getattr(getattr(self, "scheduler", None), "requests", {})
            )
            remaining_ids = [
                request_id
                for request_id in request
                if request_id not in deferred_ids or request_id in active_request_ids
            ]
            if remaining_ids:
                self.abort_requests(remaining_ids)
    ...
    def _stability_request_admission_state(self) -> str:
        """Return a feature-gated lifecycle admission state.

        Executors without the isolated lifecycle API retain their existing
        scheduling behavior.  An unknown value is treated as poisoned so a
        malformed lifecycle implementation cannot accidentally admit work.
        """
        reader = getattr(self.model_executor, "stability_request_admission_state", None)
        if not callable(reader):
            return "disabled"
        state = reader()
        return (
            state if state in {"disabled", "ready", "busy", "poisoned"} else "poisoned"
        )

    def _stability_admit_one_deferred_add(self) -> bool:
        """Admit one FIFO ADD only after the previous generation is clean."""
        if not self._stability_deferred_adds:
            return False
        state = self._stability_request_admission_state()
        if state == "busy":
            return False
        if state == "poisoned":
            return bool(self._stability_flush_deferred_adds(aborted=False))
        request, request_wave = self._stability_deferred_adds.popleft()
    ...
    def _stability_abort_deferred_requests(self, request_ids: list[str]) -> set[str]:
        """Abort only not-yet-admitted FIFO requests without model cleanup."""
        if not self._stability_deferred_adds:
            return set()
        requested = set(request_ids)
        retained: deque[tuple[Request, int]] = deque()
        aborted: set[str] = set()
        while self._stability_deferred_adds:
            request, request_wave = self._stability_deferred_adds.popleft()
            if request.request_id not in requested:
                retained.append((request, request_wave))
                continue
            aborted.add(request.request_id)
            self._send_abort_outputs_to_client(
                [request.request_id], request.client_index
            )
        self._stability_deferred_adds = retained
        return aborted

    def _stability_flush_deferred_adds(self, *, aborted: bool) -> list[str]:
        """Resolve all never-admitted requests without touching TP resources."""
        if not self._stability_deferred_adds:
            return []
        by_client: dict[int, list[str]] = defaultdict(list)
        while self._stability_deferred_adds:
            request, _ = self._stability_deferred_adds.popleft()
            by_client[request.client_index].append(request.request_id)
        for client_index, request_ids in by_client.items():
            if aborted:
                self._send_abort_outputs_to_client(request_ids, client_index)
            else:
                self._send_error_outputs_to_client(request_ids, client_index)
        return [
            request_id
            for request_ids in by_client.values()
            for request_id in request_ids
        ]
