...
class MessageQueue:
    def __init__(
        ...
        self.debug_name = name or "unnamed"
    ...
    def create_from_handle(handle: Handle, rank) -> "MessageQueue":
        ...
        self.handle = handle
        self.debug_name = handle.name or "unnamed"
    ...
    def diagnostic_snapshot(self, include_slots: bool = False) -> dict[str, Any]:
        """Return a best-effort, read-only view of this queue's SHM ring."""
        buffer = self.buffer
        result: dict[str, Any] = {
            "name": self.debug_name,
            "current_idx": self.current_idx,
            "role": "writer" if self._is_writer else "reader",
            "is_local_reader": self._is_local_reader,
            "is_remote_reader": self._is_remote_reader,
            "local_reader_rank": self.local_reader_rank,
            "has_shm_ring": buffer is not None,
        }
        if buffer is None:
            return result

        result["shm_name"] = buffer.shared_memory.name
        result["segment_bytes"] = buffer.shared_memory.size
        result["configured_segment_bytes"] = buffer.total_bytes_of_buffer

        slots: list[dict[str, Any]] = []
        free_for_writer = 0
        written_unread = 0
        fully_read = 0
        pending_reader_flags = 0
        local_reader_unread = 0
        for slot in range(buffer.max_chunks):
            with buffer.get_metadata(slot) as metadata:
                memory_fence()
                flags = [int(value) for value in metadata]
            written = bool(flags[0])
            read_count = sum(flags[1:])
            is_free = not written or read_count == buffer.n_reader
            if is_free:
                free_for_writer += 1
            elif written:
                written_unread += 1
            if written and read_count == buffer.n_reader:
                fully_read += 1
            if written:
                pending_reader_flags += buffer.n_reader - read_count
                if self._is_local_reader and flags[self.local_reader_rank + 1] == 0:
                    local_reader_unread += 1
            if include_slots:
                slots.append(
                    {
                        "slot": slot,
                        "written": written,
                        "reader_flags": flags[1:],
                        "free_for_writer": is_free,
                    }
                )

        result["ring"] = {
            "max_chunks": buffer.max_chunks,
            "n_readers": buffer.n_reader,
            "free_for_writer": free_for_writer,
            "written_unread": written_unread,
            "fully_read": fully_read,
            "pending_reader_flags": pending_reader_flags,
            "local_reader_unread": local_reader_unread,
        }
        if include_slots:
            result["slots"] = slots
        return result

    def _emit_stability_wait(self, wait_kind: str, elapsed_s: float) -> None:
        ...
        try:
            emit_stability_telemetry(
                ...
                wait_kind=wait_kind,
                elapsed_s=elapsed_s,
                queue=self.diagnostic_snapshot(include_slots=True),
            )
        except Exception:
            return
            ...
            with self.buffer.get_metadata(self.current_idx) as metadata_buffer:
                ...
                if not check():
                    ...
                    if elapsed > VLLM_RINGBUFFER_WARNING_INTERVAL * n_warning:
                        ...
                        self._emit_stability_wait("write", elapsed)
                        n_warning += 1
            ...
            while True:
                ...
                if not check():
                    ...
                    if read_timeout.should_warn():
                        ...
                        self._emit_stability_wait(
                            "read", time.monotonic() - read_timeout.started
                        )
