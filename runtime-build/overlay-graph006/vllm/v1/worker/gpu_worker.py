...
_SM120_GRAPH_MEMORY_PREFIX = "VLLM_SM120_GRAPH_MEMORY="
_SM120_ALLOCATOR_PROBE_BYTES = 128 * 1024 * 1024
_SM120_KV_BYTES_REQUIRED_FOR_262144 = 2_928_199_680
...
class Worker(WorkerBase):
    ...
    def initialize_from_config(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        if os.getenv(_SM120_NVFP4_K3_GRAPH_GATE_ENV) == "1":
            packed_size = next(
                (
                    tensor.size
            ...
            self._sm120_kv_bytes_allocated = packed_size + unpacked_size
    ...
    def compile_or_warm_up_model(self) -> CompilationTimes:
        ...
        if os.getenv(_SM120_NVFP4_K3_GRAPH_GATE_ENV) == "1":
            self._emit_sm120_graph_memory_ledger(cuda_graph_memory_bytes)
        ...
        self._emit_stability_startup_ready(cuda_graph_memory_bytes)
    ...
    def _emit_stability_startup_ready(self, cuda_graph_memory_bytes: int) -> None:
        """Record the worker's post-warmup baseline for diagnostic runs."""
        if not stability_telemetry_enabled():
            return
        try:
            emit_stability_telemetry(
                "gpu_worker_startup_ready",
                worker_rank=self.rank,
                cuda_graph_memory_bytes=cuda_graph_memory_bytes,
                runner=self.model_runner._stability_runner_snapshot(
                    include_memory=True
                ),
            )
        except Exception:
            logger.debug("Unable to emit startup stability telemetry", exc_info=True)

    def _emit_sm120_graph_memory_ledger(self, cuda_graph_memory_bytes: int) -> None:
        """Materialize the campaign probe and emit one post-capture ledger."""
        torch.accelerator.synchronize()
        allocated_before = torch.cuda.memory_allocated(self.device)
        reserved_before = torch.cuda.memory_reserved(self.device)
        free_before, total_bytes = torch.accelerator.get_memory_info()
        probe_passed = False
        probe_ptr = 0
        ...
        try:
            probe = torch.empty(
                _SM120_ALLOCATOR_PROBE_BYTES,
                dtype=torch.uint8,
                device=self.device,
            )
            probe_ptr = probe.data_ptr()
            probe.fill_(0xA5)
            torch.accelerator.synchronize()
            probe_passed = True
            ...
            # Do not inspect locals() here. On CPython 3.12 that materializes a
            # frame-locals dict which keeps the CUDA tensor alive until this
            # function returns, making the post-probe allocation check observe
            # the probe itself.
            probe = None
            torch.accelerator.synchronize()
            torch.accelerator.empty_cache()
        ...
        free_after, total_after = torch.accelerator.get_memory_info()
        if total_after != total_bytes:
            raise RuntimeError(
                "GPU total memory changed during SM120 allocator probe: "
                f"{total_bytes=} {total_after=}"
            )
        profile = getattr(self.model_runner, "_sm120_graph_profile_memory", {})
        actual = getattr(self.model_runner, "_sm120_graph_actual_memory", {})
        physical_used = total_after - free_after
        non_torch_physical = max(physical_used - reserved_after, 0)
        gdn_state_bytes = 0
        attention_kv_view_bytes = 0
        for layer in self.compilation_config.static_forward_context.values():
            ...
            if isinstance(cache, tuple):
                ...
                attention_kv_view_bytes += cache.numel() * cache.element_size()
        kv_bytes_allocated = int(getattr(self, "_sm120_kv_bytes_allocated", 0))
        ledger = {
            "schema_version": 1,
            "event": "post_capture_startup",
            "rank": self.rank,
            "estimated_graph_bytes": int(getattr(self, "cudagraph_memory_estimate", 0)),
            "throwaway_retained_graph_bytes": int(
                profile.get("throwaway_retained_graph_bytes", 0)
            ),
            "profiling_capture_peak_allocated_bytes": int(
                profile.get("profiling_capture_peak_allocated_bytes", 0)
            ),
            "actual_retained_graph_bytes": int(
                actual.get("actual_retained_graph_bytes", cuda_graph_memory_bytes)
            ),
            "actual_capture_peak_allocated_bytes": int(
                actual.get("actual_capture_peak_allocated_bytes", 0)
            ),
            "physical_free_after_capture_bytes": free_after,
            "physical_total_bytes": total_after,
            "allocator_probe_bytes": _SM120_ALLOCATOR_PROBE_BYTES,
            "allocator_probe_data_ptr": probe_ptr,
            "allocator_probe_passed": probe_passed,
            "allocator_probe_error": probe_error,
            "allocator_allocated_before_probe": allocated_before,
            "allocator_allocated_after_probe": allocated_after,
            "allocator_reserved_before_probe": reserved_before,
            "allocator_reserved_after_probe": reserved_after,
            "physical_free_before_probe_bytes": free_before,
            "kv_bytes_allocated": kv_bytes_allocated,
            "kv_bytes_required_for_262144": (_SM120_KV_BYTES_REQUIRED_FOR_262144),
            "gdn_persistent_state_view_bytes": gdn_state_bytes,
            "attention_kv_view_bytes": attention_kv_view_bytes,
            "torch_non_kv_persistent_bytes": max(
                allocated_after - kv_bytes_allocated, 0
            ),
            "non_torch_driver_tp_nccl_bytes": non_torch_physical,
        }
        logger.info(
            "%s%s",
            _SM120_GRAPH_MEMORY_PREFIX,
            json.dumps(ledger, separators=(",", ":"), sort_keys=True),
        )
        if not probe_passed or allocated_after != allocated_before:
            raise RuntimeError(
                "SM120 allocator probe did not return to its pre-probe allocation: "
                f"{probe_passed=} {allocated_before=} {allocated_after=} "
                f"{probe_error=}"
            )
