...
SM120_GRAPH_WORKSPACE_ALIGNMENT = 256
SM120_GRAPH_WORKSPACE_MARGIN = 1024 * 1024
SM120_GRAPH_TARGET_FLASH_ATTN_LAYERS = 16
...
def _sm120_nvfp4_k3_native_fa2_enabled(vllm_config: VllmConfig) -> bool:
    cache_config = vllm_config.cache_config
    speculative_config = vllm_config.speculative_config
    attention_config = vllm_config.attention_config
    return (
        os.getenv("VLLM_SM120_NVFP4_K3_NATIVE") == "1"
        ...
        and attention_config.use_trtllm_attention is False
        and not attention_config.use_non_causal
    )


def _partition_sm120_mtp_layer_names(
    layer_names: list[str],
) -> tuple[list[str], list[str]]:
    """Split target layers from the Qwen3.5 MTP draft layer.

    The hybrid KV-cache grouping includes the single MTP full-attention layer
    in the same uniform group as the 16 target full-attention layers.  The
    proposer also creates its own draft-only metadata builder, so group size is
    not itself a target-layer count.
    """
    draft_layer_names = [
        name for name in layer_names if name.startswith("mtp.") or ".mtp." in name
    ]
    draft_layer_name_set = set(draft_layer_names)
    target_layer_names = [
        name for name in layer_names if name not in draft_layer_name_set
    ]
    return target_layer_names, draft_layer_names


def _sm120_mtp_builder_role(layer_names: list[str]) -> str:
    target_layer_names, draft_layer_names = _partition_sm120_mtp_layer_names(
        layer_names
    )
    if (
        len(target_layer_names) == SM120_GRAPH_TARGET_FLASH_ATTN_LAYERS
        and len(draft_layer_names) == 1
    ):
        return "target"
    if not target_layer_names and len(draft_layer_names) == 1:
        return "drafter"
    raise RuntimeError(
        "The isolated SM120 NVFP4 K3 route requires a target metadata group "
        f"with {SM120_GRAPH_TARGET_FLASH_ATTN_LAYERS} target FlashInfer layers "
        "plus one grouped MTP draft layer, or its one-layer drafter group; got "
        f"target={len(target_layer_names)}, draft={len(draft_layer_names)}, "
        f"total={len(layer_names)}"
    )


def _builder_cuda_graph_enabled(
    cudagraph_mode: CUDAGraphMode, sm120_builder_role: str | None
) -> bool:
    """Opt in only the isolated drafter's one-token decode wrapper."""
    return (
        cudagraph_mode.decode_mode() == CUDAGraphMode.FULL
        and (sm120_builder_role != "drafter"
             or os.getenv("VLLM_SM120_MTP_DECODE_GRAPH") == "1")
    )


def _require_native_multi_token_decode_api() -> None:
    q_len_required = {
        "BatchDecodeWithPagedKVCacheWrapper._plan_impl": getattr(
            BatchDecodeWithPagedKVCacheWrapper, "_plan_impl", None
        ),
        "BatchDecodeWithPagedKVCacheWrapper.run": (
            BatchDecodeWithPagedKVCacheWrapper.run
        ),
        "flashinfer.decode.fast_decode_plan": fast_decode_plan,
    }
    missing = []
    for name, func in q_len_required.items():
        if func is None or "q_len_per_req" not in inspect.signature(func).parameters:
            missing.append(name)
    workspace_required = {
        "BatchDecodeWithPagedKVCacheWrapper.workspace_size": getattr(
            BatchDecodeWithPagedKVCacheWrapper, "workspace_size", None
        ),
        "BatchDecodeWithPagedKVCacheWrapper.reset_workspace_buffer": getattr(
            BatchDecodeWithPagedKVCacheWrapper, "reset_workspace_buffer", None
        ),
        "BatchPrefillWithPagedKVCacheWrapper.workspace_size": getattr(
            BatchPrefillWithPagedKVCacheWrapper, "workspace_size", None
        ),
        "BatchPrefillWithPagedKVCacheWrapper.reset_workspace_buffer": getattr(
            BatchPrefillWithPagedKVCacheWrapper, "reset_workspace_buffer", None
        ),
    }
    missing.extend(name for name, func in workspace_required.items() if func is None)
    if missing:
        raise RuntimeError(
            "VLLM_SM120_NVFP4_K3_NATIVE requires FlashInfer's native "
            "q_len_per_req API and caller-owned workspace API; missing support in "
            + ", ".join(missing)
        )


def _get_native_decode_shape(
    num_actual_tokens: int,
    num_decodes: int,
    num_prefill_tokens: int,
) -> tuple[int, int]:
    if num_decodes <= 0:
        raise ValueError(f"Expected at least one decode request, got {num_decodes=}")
    num_decode_tokens = num_actual_tokens - num_prefill_tokens
    if num_decode_tokens <= 0 or num_decode_tokens % num_decodes != 0:
        raise ValueError(
            "Native FlashInfer decode requires a positive uniform query length; "
            f"got {num_actual_tokens=}, {num_decodes=}, {num_prefill_tokens=}"
        )
    return num_decodes, num_decode_tokens // num_decodes


def _workspace_allocation_size(required: int) -> int:
    aligned = cdiv(required, SM120_GRAPH_WORKSPACE_ALIGNMENT)
    return aligned * SM120_GRAPH_WORKSPACE_ALIGNMENT + SM120_GRAPH_WORKSPACE_MARGIN


def _workspace_probe_lengths(max_model_len: int, page_size: int) -> list[int]:
    lengths = {
        4,
        min(236_000, max_model_len),
        min(260_927, max_model_len),
        min(262_143, max_model_len),
        min(262_144, max_model_len),
        max_model_len,
    }
    for length in tuple(lengths):
        lower = length // page_size * page_size
        upper = cdiv(length, page_size) * page_size
        for boundary in (lower, upper):
            lengths.update((boundary - 1, boundary, boundary + 1))
    return sorted(length for length in lengths if 4 <= length <= max_model_len)


def _workspace_prefill_query_lengths(
    ...
    """Probe split-KV transition shapes for the pinned FA2 scheduler.

    A full 2048-token chunk has enough Q tiles to avoid split-KV and therefore
    does not expose the largest prefill scratch requirement. In particular, a
    short final chunk can use all ``2 * SM / KV-head`` split slots. Include the
    observed qlen=5 failure plus the exact tile/split boundaries for the
    campaign's head_dim=256 FA2 scheduler (CTA Q tiles 16 and 64).
    """
    if num_qo_heads <= 0 or num_kv_heads <= 0 or num_sms <= 0:
        raise ValueError("FA2 workspace probe dimensions must be positive")
    if num_qo_heads % num_kv_heads:
        raise ValueError("FA2 workspace probe requires integral GQA groups")
    if head_dim != 256:
        raise ValueError("The isolated SM120 FA2 workspace probe requires head_dim=256")

    gqa_group_size = num_qo_heads // num_kv_heads
    max_split_batch = (2 * num_sms) // num_kv_heads
    first_wide_tile_q_len = 16 // gqa_group_size + 1
    last_two_way_split_q_len = (max_split_batch // 2) * 64 // gqa_group_size
    last_below_capacity_q_len = (max_split_batch - 1) * 64 // gqa_group_size
    candidates = {
        1,
        2,
        first_wide_tile_q_len,
        4,
        5,
        last_two_way_split_q_len - 1,
        last_two_way_split_q_len,
        last_two_way_split_q_len + 1,
        last_below_capacity_q_len,
        last_below_capacity_q_len + 1,
        max_num_batched_tokens,
    }
    return sorted(
        query_len
        for query_len in candidates
        if 1 <= query_len <= max_num_batched_tokens
    )
...
class FlashInferBackend(AttentionBackend):
    ...
    def get_required_kv_cache_layout(cls) -> KVCacheLayoutType | None:
        ...
        if capability is not None and capability.major == 10:
            return "HND"
        # NVFP4 KV on consumer Blackwell packs each K/V side as contiguous
        # [data | scale] regions. That carve is byte-coherent only under the
        # head-major HND layout; NHD interleaves head rows and corrupts the
        # scale-factor views.
...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def __init__(
        ...
        sm120_native_gate = _sm120_nvfp4_k3_native_fa2_enabled(vllm_config)
        sm120_builder_role = (
            _sm120_mtp_builder_role(layer_names) if sm120_native_gate else None
        )
        self.enable_cuda_graph = _builder_cuda_graph_enabled(
            self.compilation_config.cudagraph_mode, sm120_builder_role
        )
        ...
        self.sm120_nvfp4_k3_native_campaign = (
            sm120_native_gate and self.use_fa2_nvfp4_kv and not self.use_dcp
        )
        if not self.sm120_nvfp4_k3_native_campaign:
            sm120_builder_role = None
        self.is_sm120_mtp_drafter = sm120_builder_role == "drafter"
        self._sm120_mtp_workspace_ledger_paths: set[str] = set()
        self._sm120_mtp_workspace_wrappers: dict[
            str,
            BatchDecodeWithPagedKVCacheWrapper | BatchPrefillWithPagedKVCacheWrapper,
        ] = {}
        self._sm120_mtp_workspace_lock: dict[str, tuple[int, int, int, int]] = {}
        self._sm120_preparing_mtp_workspaces = False
        self.use_sm120_nvfp4_k3_native_fa2 = sm120_builder_role == "target"
        if self.use_sm120_nvfp4_k3_native_fa2:
            _require_native_multi_token_decode_api()

        if self.is_kvcache_nvfp4 and get_kv_cache_layout() != "HND":
            # The g99 cache-layout API resolves to a global HND/NHD string.
            # The NVFP4 [data | scale] carve is byte-coherent only for HND.
            raise ValueError(
        ...
        ) or self.use_sm120_nvfp4_k3_native_fa2
        ...
        if self.use_sm120_nvfp4_k3_native_fa2:
            logger.info_once(
                "SM120_NVFP4_K3_NATIVE_FA2 enabled: logical_batch=requests, "
                "q_len_per_req=4, dcp=1"
            )
        ...
        if self.use_sm120_nvfp4_k3_native_fa2:
            self._prepare_sm120_native_workspaces()
        elif self.is_sm120_mtp_drafter:
            self._prepare_sm120_mtp_workspaces()
    ...
    def get_cudagraph_support(
        ...
        # SM12x NVFP4 decode normally retains the conservative single-token
        # contract. The isolated K3 experiment opts into FlashInfer's native
        # FA2 q_len_per_req=4 path.
        cache_config = vllm_config.cache_config
        if (
            ...
            if _sm120_nvfp4_k3_native_fa2_enabled(vllm_config):
                _require_native_multi_token_decode_api()
    ...
    def set_workspace_buffer(self, workspace_buffer: torch.Tensor):
        if self.sm120_nvfp4_k3_native_campaign:
            return
    ...
    def _workspace_metadata(
        ...
        indptr = torch.tensor([0, num_pages], dtype=torch.int32, device="cpu")
        indices = self.paged_kv_indices.gpu[:num_pages]
        last_page = torch.tensor([last_page_len], dtype=torch.int32, device="cpu")
        return indptr, indices, last_page

    def _query_decode_workspace(
        self,
        wrapper: BatchDecodeWithPagedKVCacheWrapper,
        seq_len: int,
        q_len_per_req: int = 4,
    ) -> tuple[int, int]:
        indptr, indices, last_page = self._workspace_metadata(seq_len)
    ...
    def _query_prefill_workspace(
        self,
        wrapper: BatchPrefillWithPagedKVCacheWrapper,
        seq_len: int,
        query_len: int,
    ) -> tuple[int, int]:
        paged_kv_indptr, indices, last_page = self._workspace_metadata(seq_len)
        if not 1 <= query_len <= min(self.max_num_batched_tokens, seq_len):
            raise ValueError(
                "Invalid FA2 prefill workspace shape: "
                f"{query_len=}, {seq_len=}, "
                f"max_num_batched_tokens={self.max_num_batched_tokens}"
            )
        qo_indptr = torch.tensor([0, query_len], dtype=torch.int32, device="cpu")
        return wrapper.workspace_size(
            qo_indptr=qo_indptr,
    ...
    def _prepare_sm120_native_workspaces(self) -> None:
        # workspace_size() only uses this as a device/stream selector. Avoid
        # materializing the legacy fixed-size float workspace before querying.
        self._workspace_buffer = torch.empty(
            SM120_GRAPH_WORKSPACE_ALIGNMENT,
            dtype=torch.uint8,
            device=self.device,
        )
        prefill_wrapper = self._get_prefill_wrapper()
        assert isinstance(prefill_wrapper, BatchPrefillWithPagedKVCacheWrapper)
        if prefill_wrapper.is_cuda_graph_enabled:
            raise RuntimeError("The isolated SM120 target prefill must remain eager")
        wrappers: dict[
            str,
            BatchDecodeWithPagedKVCacheWrapper | BatchPrefillWithPagedKVCacheWrapper,
        ] = {
            "target_prefill": prefill_wrapper,
            "target_decode_eager": self._get_decode_wrapper(
                batch_size=1, use_cudagraph=False
            ),
        }
        if self.enable_cuda_graph:
            wrappers["target_verify_graph"] = self._get_decode_wrapper(
                batch_size=1, use_cudagraph=True
            )

        lengths = _workspace_probe_lengths(
            self.model_config.max_model_len, self.page_size
        )
        num_sms = torch.cuda.get_device_properties(self.device).multi_processor_count
        prefill_query_lens = _workspace_prefill_query_lengths(
            self.max_num_batched_tokens,
            self.num_qo_heads,
            self.num_kv_heads,
            self.head_dim,
        ...
        max_float_required = 0
        for name, wrapper in wrappers.items():
            by_length: dict[str, list[int]] = {}
            max_int_required = 0
            wrapper_float_required = 0
            for length in lengths:
                if isinstance(wrapper, BatchPrefillWithPagedKVCacheWrapper):
                    shape_requirements = [
                        self._query_prefill_workspace(wrapper, length, query_len)
                        for query_len in prefill_query_lens
                        if query_len <= length
                    ]
                    float_required = max(value[0] for value in shape_requirements)
                    int_required = max(value[1] for value in shape_requirements)
                else:
                    float_required, int_required = self._query_decode_workspace(
                        wrapper, length
                    )
                by_length[str(length)] = [float_required, int_required]
                wrapper_float_required = max(wrapper_float_required, float_required)
                max_int_required = max(max_int_required, int_required)
            max_float_required = max(max_float_required, wrapper_float_required)
            requirements[name] = {
                "required_float_high_water": wrapper_float_required,
                "required_int_high_water": max_int_required,
                "by_seq_len": by_length,
            }
            if isinstance(wrapper, BatchPrefillWithPagedKVCacheWrapper):
                requirements[name]["probe_query_lens"] = prefill_query_lens

        allocated_before = torch.cuda.memory_allocated(self.device)
        float_allocated = _workspace_allocation_size(max_float_required)
        shared_float_workspace = torch.empty(
            float_allocated,
            dtype=torch.uint8,
            device=self.device,
        )
        lock: dict[str, tuple[int, int, int, int]] = {}
        for name, wrapper in wrappers.items():
            required = requirements[name]
            int_allocated = _workspace_allocation_size(
                int(required["required_int_high_water"])
            )
            ...
            wrapper.reset_workspace_buffer(shared_float_workspace, int_workspace)
            required["allocated_float"] = float_allocated
            required["allocated_int"] = int_allocated
            required["float_data_ptr"] = shared_float_workspace.data_ptr()
            required["int_data_ptr"] = int_workspace.data_ptr()
            lock[name] = (
                shared_float_workspace.data_ptr(),
                shared_float_workspace.numel(),
                int_workspace.data_ptr(),
                int_workspace.numel(),
            )

        self._workspace_buffer = shared_float_workspace
        self._sm120_workspace_wrappers = wrappers
        self._sm120_workspace_lock = lock
        ...
        ledger = {
            ...
            "shared_with_mtp_drafter": False,
            "layout": get_kv_cache_layout(),
            "page_size": self.page_size,
            "num_qo_heads_local": self.num_qo_heads,
            "num_kv_heads_local": self.num_kv_heads,
            "head_dim": self.head_dim,
            "num_sms": num_sms,
            "q_dtype_prefill": str(self.q_data_type_prefill),
            "q_dtype_decode": str(self.q_data_type_decode),
            "kv_dtype": str(self.kv_cache_dtype),
            "prefill_chunk_tokens": self.max_num_batched_tokens,
            "prefill_probe_query_lens": prefill_query_lens,
            "verify_q_len_per_req": 4,
            "probe_seq_lens": lengths,
            "shared_float_required_high_water": max_float_required,
            "shared_float_allocated": float_allocated,
            "cuda_allocated_before": allocated_before,
            "cuda_allocated_after": allocated_after,
            "wrappers": requirements,
        }
        logger.info(
            "VLLM_SM120_WORKSPACE_LEDGER=%s",
            json.dumps(ledger, sort_keys=True, separators=(",", ":")),
        )

    def _prepare_sm120_mtp_workspaces(self) -> None:
        """Measure and lock the pinned MTP prefill and eager-decode workspaces."""
        self._workspace_buffer = torch.empty(
            SM120_GRAPH_WORKSPACE_ALIGNMENT,
            dtype=torch.uint8,
            device=self.device,
        )
        self._sm120_preparing_mtp_workspaces = True
        try:
            prefill_wrapper = self._get_prefill_wrapper()
            decode_wrapper = self._get_decode_wrapper(batch_size=1, use_cudagraph=False)
            graph_wrapper = (
                self._get_decode_wrapper(batch_size=1, use_cudagraph=True)
                if os.getenv("VLLM_SM120_MTP_DECODE_GRAPH") == "1" else None
            )
        finally:
            self._sm120_preparing_mtp_workspaces = False
        if not isinstance(prefill_wrapper, BatchPrefillWithPagedKVCacheWrapper):
            raise RuntimeError(
                "The isolated SM120 MTP drafter requires native FA2 prefill"
            )
        if prefill_wrapper.is_cuda_graph_enabled:
            raise RuntimeError(
                "The isolated SM120 MTP drafter prefill must remain eager"
            )
        if not isinstance(decode_wrapper, BatchDecodeWithPagedKVCacheWrapper):
            raise RuntimeError(
                "The isolated SM120 MTP drafter requires native FA2 decode"
            )
        ...
        if graph_wrapper is not None:
            wrappers["decode_graph"] = graph_wrapper
        lengths = _workspace_probe_lengths(
            self.model_config.max_model_len, self.page_size
        )
        num_sms = torch.cuda.get_device_properties(self.device).multi_processor_count
        ...
        for name, wrapper in wrappers.items():
            ...
            for length in lengths:
                if name == "prefill":
                    shape_requirements = [
                        self._query_prefill_workspace(
                            prefill_wrapper, length, query_len
                        )
                        for query_len in prefill_query_lens
                        if query_len <= length
                    ]
                    ...
                    float_required, int_required = self._query_decode_workspace(
                        wrapper, length, q_len_per_req=1
                    )
                by_length[str(length)] = [float_required, int_required]
                wrapper_float_required = max(wrapper_float_required, float_required)
                max_int_required = max(max_int_required, int_required)
        ...
        for name, wrapper in wrappers.items():
            ...
            required["float_data_ptr"] = shared_float_workspace.data_ptr()
            required["int_data_ptr"] = int_workspace.data_ptr()
            total_int_allocated += int_allocated
            lock[name] = (
                shared_float_workspace.data_ptr(),
                shared_float_workspace.numel(),
                int_workspace.data_ptr(),
        ...
        self._workspace_buffer = shared_float_workspace
        self._sm120_mtp_workspace_wrappers = wrappers
        self._sm120_mtp_workspace_lock = lock
        self._sm120_mtp_workspace_ledger_paths.update(wrappers)
        allocated_after = torch.cuda.memory_allocated(self.device)
        ledger = {
            "owner": "mtp_drafter",
            "rank": get_tensor_model_parallel_rank(),
            "path": "prefill+decode",
            "attention_path": "prefill+decode",
            "backend": "fa2",
            "materialized": True,
            "workspace_locked": True,
            "shared_with_target": False,
            "layout": get_kv_cache_layout(),
            "page_size": self.page_size,
            "num_qo_heads_local": self.num_qo_heads,
            "num_kv_heads_local": self.num_kv_heads,
            ...
            "shared_float_required_high_water": max_float_required,
            "shared_float_allocated": float_allocated,
            "data_ptr": shared_float_workspace.data_ptr(),
            "float_data_ptr": shared_float_workspace.data_ptr(),
            "float_allocated": float_allocated,
            "int_allocated": total_int_allocated,
            "allocated_bytes": float_allocated + total_int_allocated,
            "cuda_allocated_before": allocated_before,
            "cuda_allocated_after": allocated_after,
            "cuda_graph": graph_wrapper is not None,
            "wrappers": requirements,
        }
        logger.info(
            "VLLM_SM120_WORKSPACE_LEDGER=%s",
        ...
        self._assert_sm120_mtp_workspaces_locked()

    def _assert_sm120_native_workspaces_locked(self) -> None:
        for name, wrapper in self._sm120_workspace_wrappers.items():
            expected = self._sm120_workspace_lock[name]
            actual = (
                wrapper._float_workspace_buffer.data_ptr(),
                wrapper._float_workspace_buffer.numel(),
                wrapper._int_workspace_buffer.data_ptr(),
                wrapper._int_workspace_buffer.numel(),
            )
            if actual != expected:
                raise RuntimeError(
                    "SM120 target workspace changed after preallocation: "
                    f"{name=} {expected=} {actual=}"
                )

    def _emit_sm120_mtp_workspace_ledger(
        self,
        wrapper: (
            BatchDecodeWithPagedKVCacheWrapper | BatchPrefillWithPagedKVCacheWrapper
        ),
        *,
        ...
        if (
            not self.is_sm120_mtp_drafter
            or self._sm120_preparing_mtp_workspaces
            or path in self._sm120_mtp_workspace_ledger_paths
        ):
            return

        # The wrapper owns its int workspace. Keeping the wrapper in
        # _prefill_wrapper/_decode_wrapper therefore also keeps both pointers
        # alive for the complete drafter lifetime.
        float_workspace = wrapper._float_workspace_buffer
        int_workspace = wrapper._int_workspace_buffer
        lock = (
            float_workspace.data_ptr(),
            float_workspace.numel(),
            int_workspace.data_ptr(),
            int_workspace.numel(),
        )
        self._sm120_mtp_workspace_wrappers[path] = wrapper
        self._sm120_mtp_workspace_lock[path] = lock
        float_allocated = float_workspace.numel() * float_workspace.element_size()
        int_allocated = int_workspace.numel() * int_workspace.element_size()
        ledger = {
            "owner": "mtp_drafter",
            "rank": get_tensor_model_parallel_rank(),
            "path": path,
            ...
            "q_dtype_decode": str(self.q_data_type_decode),
            "kv_dtype": str(self.kv_cache_dtype),
            "data_ptr": float_workspace.data_ptr(),
            "allocated_bytes": float_allocated + int_allocated,
            "float_data_ptr": float_workspace.data_ptr(),
            "float_allocated": float_allocated,
            "int_data_ptr": int_workspace.data_ptr(),
            "int_allocated": int_allocated,
        ...
        self._sm120_mtp_workspace_ledger_paths.add(path)

    def _assert_sm120_mtp_workspaces_locked(self) -> None:
        for path, wrapper in self._sm120_mtp_workspace_wrappers.items():
            expected = self._sm120_mtp_workspace_lock[path]
            actual = (
                wrapper._float_workspace_buffer.data_ptr(),
                wrapper._float_workspace_buffer.numel(),
                wrapper._int_workspace_buffer.data_ptr(),
            ...
            if actual != expected:
                raise RuntimeError(
                    "SM120 MTP drafter workspace changed after materialization: "
    ...
    def _get_prefill_wrapper(
        ...
        if self._prefill_wrapper is None:
            if self.use_dcp:
                ...
                if self.is_sm120_mtp_drafter:
                    self._emit_sm120_mtp_workspace_ledger(
                        self._prefill_wrapper,
                        path="prefill",
                        backend=backend,
                    )
    ...
    def _get_decode_wrapper(self, batch_size: int, use_cudagraph: bool = False):
        ...
        if decode_wrapper is None:
            if hasattr(self, "_sm120_workspace_lock"):
                raise RuntimeError(
                    "SM120 target workspace wrapper creation attempted after "
                    "workspace lock"
                )
            if use_cudagraph:
    ...
    def build(
        ...
        if self.use_sm120_nvfp4_k3_native_fa2:
            self._assert_sm120_native_workspaces_locked()
        if self.is_sm120_mtp_drafter:
            self._assert_sm120_mtp_workspaces_locked()
        ...
        if num_decodes > 0:
            if decode_with_flashinfer_trtllm_api:
                ...
                logical_batch_size = num_decode_tokens
                q_len_per_req = 1
                if self.use_sm120_nvfp4_k3_native_fa2:
                    logical_batch_size, q_len_per_req = _get_native_decode_shape(
                        num_actual_tokens=num_actual_tokens,
                        num_decodes=num_decodes,
                        num_prefill_tokens=num_prefill_tokens,
                    )
                    if num_decode_tokens != logical_batch_size * q_len_per_req:
                        raise ValueError(
                            "FlashInfer decode token accounting disagrees with "
                            "unpadded metadata: "
                            f"{num_decode_tokens=}, {num_actual_tokens=}, "
                            f"{num_prefill_tokens=}, {num_decodes=}"
                        )
                    if q_len_per_req > 4:
                        raise ValueError(
                            "The SM120 NVFP4 K3 native route supports query "
                            f"lengths up to 4, got {q_len_per_req=}"
                        )
                use_cudagraph = (
                    ...
                    and (
                        not self.use_sm120_nvfp4_k3_native_fa2
                        or q_len_per_req == 4
                        or (self.is_sm120_mtp_drafter and q_len_per_req == 1
                            and os.getenv("VLLM_SM120_MTP_DECODE_GRAPH") == "1")
                    )
                )
                if (
                    self.use_sm120_nvfp4_k3_native_fa2
                    and use_cudagraph
                    and torch.any(
                        seq_lens_cpu[:logical_batch_size] < q_len_per_req
                    ).item()
                ):
                    raise ValueError(
                        "SM120 NVFP4 K3 CUDA Graph capture requires dummy KV "
                        f"length >= {q_len_per_req}"
                    )

                decode_wrapper = self._get_decode_wrapper(
                    logical_batch_size, use_cudagraph
                )
                ...
                fast_plan_decode(
                    decode_wrapper,
                    indptr_cpu=self.paged_kv_indptr.cpu[: logical_batch_size + 1],
...
class FlashInferImpl(AttentionImpl):
    ...
    def __init__(
        ...
        vllm_config = get_current_vllm_config_or_none()
        self._sm120_graph_validation_enabled = (
            vllm_config is not None
            and os.getenv("VLLM_SM120_GRAPH_VALIDATION_DIR") is not None
            and _sm120_nvfp4_k3_native_fa2_enabled(vllm_config)
            and self.use_fa2_nvfp4_kv
        )
        if self._sm120_graph_validation_enabled:
            # FULL replay does not execute this Python method. Keep the tensor
            # captured by the graph separate from later eager prefill tensors;
            # the captured view is updated in-place on every replay.
            self._sm120_graph_validation_records: dict[str, dict[str, object]] = {}
    ...
    def forward(
        ...
        if num_decode_tokens > 0:
            ...
            if not decode_with_flashinfer_trtllm_api:
                ...
                assert attn_metadata.num_decodes * q_len_per_req == num_decode_tokens, (
                    "Native FlashInfer decode shape changed after planning: "
                    f"num_decodes={attn_metadata.num_decodes}, "
                    f"{q_len_per_req=}, {num_decode_tokens=}"
                )
        ...
        if self._sm120_graph_validation_enabled:
            route = "graph" if torch.cuda.is_current_stream_capturing() else "eager"
            record = self._sm120_graph_validation_records.setdefault(route, {})
            validation_output = output[:num_actual_tokens]
            if route == "graph":
                # Capture a dedicated validation sink. The clone is part of the
                # graph, so replay refreshes it even when an intermediate output
                # buffer is subsequently reused elsewhere in the forward.
                validation_output = validation_output.clone()
            record.update(
        ...
        if self.kv_sharing_target_layer_name is None:
            if self._sm120_graph_validation_enabled:
                ...
                record.update(
                    ...
                    kv_write_tokens=slot_mapping.numel(),
                )
