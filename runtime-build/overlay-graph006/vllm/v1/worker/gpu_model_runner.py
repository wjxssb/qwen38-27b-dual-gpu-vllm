...
from vllm.v1.worker.sm120_graph_validation import (
    record_after_accepted_commit as record_sm120_after_accepted_commit,
)
from vllm.v1.worker.sm120_graph_validation import (
    record_after_forward as record_sm120_after_forward,
)
...
_SM120_NVFP4_K3_GRAPH_GATE_ENV = "VLLM_SM120_NVFP4_K3_GRAPH"
_SM120_NVFP4_K3_GRAPH_RECORD_PREFIX = "VLLM_GRAPH_CANDIDATE_CONTRACT="
_SM120_GRAPH_VALIDATION_DIR_ENV = "VLLM_SM120_GRAPH_VALIDATION_DIR"
_SM120_GRAPH_DISPATCH_TELEMETRY_ENV = "VLLM_SM120_GRAPH_DISPATCH_TELEMETRY"
_SM120_GRAPH_RUNTIME_DISPATCH_PREFIX = "VLLM_GRAPH_RUNTIME_DISPATCH="


def _cudagraph_descriptors(
    dispatcher: CudagraphDispatcher,
) -> list[dict[str, int | str | bool | None]]:
    return [
        {
            "mode": mode.name,
            "batch_size": descriptor.num_reqs,
            "tokens": descriptor.num_tokens,
...
def _validate_sm120_nvfp4_k3_graph_resolution(record: dict[str, Any]) -> None:
    expected_backends = {
        "FlashInferBackend": AttentionCGSupport.UNIFORM_BATCH.name,
        "GDNAttentionBackend": AttentionCGSupport.UNIFORM_BATCH.name,
    }
    expected_target_descriptors = [
        {
            "mode": CUDAGraphMode.FULL.name,
            "batch_size": 1,
            "tokens": 4,
            "uniform": True,
            "has_lora": False,
    ...
    expected = {
        "backend_capabilities": expected_backends,
        "aggregate": AttentionCGSupport.UNIFORM_BATCH.name,
        "use_v2_model_runner": False,
        "compilation_mode": CompilationMode.NONE.name,
        "requested_mode": CUDAGraphMode.FULL_DECODE_ONLY.name,
        "resolved_mode": CUDAGraphMode.FULL_DECODE_ONLY.name,
        "uniform_decode_query_len": 4,
        "capture_sizes": [4],
        "max_capture_size": 4,
        "target_descriptors": expected_target_descriptors,
        "drafter_descriptors": [],
    }
    mismatches = {
    ...
    if mismatches:
        raise RuntimeError(
            "SM120 NVFP4 K3 CUDA graph resolution gate failed: "
...
class GPUModelRunner(
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> Callable | None:
        ...
        if scheduler_output.finished_req_ids:
            self._emit_stability_stage(
                "finished_request_state_removed",
                scheduler_output,
                include_memory=True,
            )
    ...
    def _is_uniform_decode(
        ...
        shape_is_uniform_decode = (
            max_num_scheduled_tokens == uniform_decode_query_len
            and num_tokens == max_num_scheduled_tokens * num_reqs
        )
        if not shape_is_uniform_decode:
            return False
        ...
        return bool(np.all(computed_tokens >= prompt_tokens))

    def _record_sm120_graph_runtime_dispatch(
        self,
        *,
        ...
        is_fresh_prompt: bool,
        is_spec_verify: bool,
    ) -> None:
        if not (
            os.getenv(_SM120_GRAPH_VALIDATION_DIR_ENV)
            or os.getenv(_SM120_GRAPH_DISPATCH_TELEMETRY_ENV) == "1"
        ):
            return

        graph_gate_enabled = os.getenv(_SM120_NVFP4_K3_GRAPH_GATE_ENV) == "1"
        if is_fresh_prompt:
            phase = "fresh_prompt"
            expected_mode = CUDAGraphMode.NONE
        elif (
            graph_gate_enabled
            and is_spec_verify
            and num_reqs == 1
            ...
            phase = "spec_verify"
            expected_mode = CUDAGraphMode.FULL
        elif is_spec_verify:
            phase = "spec_verify_fallback"
            expected_mode = CUDAGraphMode.NONE
        else:
            phase = "other"
            expected_mode = CUDAGraphMode.NONE
        ...
        if runtime_mode != expected_mode:
            raise RuntimeError(
                f"SM120 graph validation dispatch mismatch for {phase}: "
                f"expected {expected_mode.name}, got {runtime_mode.name}"
            )
        key = (phase, runtime_mode.name, num_reqs, max_query_len, num_actual_tokens)
        seen = getattr(self, "_sm120_graph_dispatch_seen", set())
        if key not in seen:
            logger.info(
                "%s%s",
                _SM120_GRAPH_RUNTIME_DISPATCH_PREFIX,
                json.dumps(record, separators=(",", ":"), sort_keys=True),
            )
            seen.add(key)
            self._sm120_graph_dispatch_seen = seen
    ...
    def execute_model(
        ...
        with (
            ...
            self._record_sm120_graph_runtime_dispatch(
                ...
                is_spec_verify=bool(scheduler_output.scheduled_spec_decode_tokens),
            )
        ...
        forward_trace_scope = (
            "gpu_model_runner: target_verifier"
            if sm120_target_verify
            else "gpu_model_runner: forward"
        )
        with (
            set_forward_context(
                attn_metadata,
            ...
            record_function_or_nullcontext(forward_trace_scope),
        ...
        record_sm120_after_forward(
            self,
            attn_metadata=attn_metadata,
            slot_mappings=slot_mappings,
            num_actual_tokens=num_tokens_unpadded,
            runtime_mode=cudagraph_mode.name,
        )
    ...
    def _stability_runner_snapshot(
        self, include_memory: bool = False
    ) -> dict[str, Any]:
        event_status: dict[str, bool | None] = {}
        for name in (
            ...
            if event is None:
                event_status[name] = None
                continue
            try:
                event_status[name] = bool(event.query())
            except RuntimeError:
                event_status[name] = None

        stream_status: dict[str, bool | None] = {}
        for name in (
            ...
            if stream is None:
                stream_status[name] = None
                continue
            try:
                stream_status[name] = bool(stream.query())
            except RuntimeError:
                stream_status[name] = None

        snapshot: dict[str, Any] = {
            "active_request_count": len(self.requests),
            "active_request_ids": sorted(self.requests),
            "input_batch_request_count": self.input_batch.num_reqs,
            "input_batch_request_ids": sorted(self.input_batch.req_id_to_index),
            "input_batch_req_id_to_index": dict(
                sorted(self.input_batch.req_id_to_index.items())
            ),
            "input_batch_free_slot_count": (
                self.input_batch.max_num_reqs - self.input_batch.num_reqs
            ),
            "input_batch_max_num_reqs": self.input_batch.max_num_reqs,
            "execute_model_state_present": self.execute_model_state is not None,
            "kv_connector_output_present": self.kv_connector_output is not None,
            "mamba_state_index_count": len(self.mamba_state_idx),
            "mamba_state_indices": dict(sorted(self.mamba_state_idx.items())),
            "use_async_scheduling": self.use_async_scheduling,
            "use_async_spec_decode": self.use_async_spec_decode,
            "tp_collective": self._stability_tp_collective_snapshot(),
            "event_ready": event_status,
            "stream_ready": stream_status,
        }
        if include_memory:
            try:
                ...
                snapshot["cuda_memory"] = {
                    ...
                    "default_stream_ready": bool(torch.cuda.current_stream().query()),
                }
            except RuntimeError as exc:
                snapshot["cuda_memory_error"] = type(exc).__name__
        return snapshot

    @staticmethod
    def _stability_tp_collective_snapshot() -> dict[str, int | None]:
        ...
        try:
            ...
            snapshot["rank"] = int(tp_group.rank)
            snapshot["world_size"] = int(tp_group.world_size)
            get_sequence_number = getattr(
                tp_group.device_group, "_get_sequence_number_for_group", None
            )
            if callable(get_sequence_number):
                snapshot["sequence_number"] = int(get_sequence_number())
        except Exception:
            pass
        return snapshot

    def _emit_stability_stage(
        self,
        stage: str,
        scheduler_output: "SchedulerOutput",
        *,
        ...
        try:
            ...
            emit_stability_telemetry(
                "gpu_model_runner_stage",
                stage=stage,
                request_progress=request_progress,
                runner=self._stability_runner_snapshot(include_memory),
            )
        except Exception:
            return
    ...
    def sample_tokens(
        ...
        self._emit_stability_stage("sample_enter", scheduler_output)
        ...
        self._emit_stability_stage("sampler_begin", scheduler_output)
        with record_function_or_nullcontext("gpu_model_runner: sample"):
            sampler_output = self._sample(logits, spec_decode_metadata)
        self._emit_stability_stage("sampler_complete", scheduler_output)

        self._update_states_after_model_execute(
            sampler_output.sampled_token_ids, scheduler_output
        )
        record_sm120_after_accepted_commit(self)
        self._emit_stability_stage("accepted_state_committed", scheduler_output)
        if self.use_async_scheduling:
            pp = get_pp_group()
        ...
        def propose_draft_token_ids(sampled_token_ids):
            ...
            self._emit_stability_stage("mtp_draft_begin", scheduler_output)
            ...
            self._emit_stability_stage("mtp_draft_complete", scheduler_output)
        ...
        self._emit_stability_stage("draft_phase_complete", scheduler_output)

        self._emit_stability_stage("bookkeeping_begin", scheduler_output)
        ...
        self._emit_stability_stage("bookkeeping_complete", scheduler_output)
        ...
        self._emit_stability_stage("eplb_begin", scheduler_output)
        with record_function_or_nullcontext("gpu_model_runner: eplb"):
            self.eplb_step()
        self._emit_stability_stage("eplb_complete", scheduler_output)
        ...
        self._emit_stability_stage(
            "sample_return", scheduler_output, include_memory=True
        )
    ...
    def profile_cudagraph_memory(self) -> int:
        ...
        sm120_graph_campaign = os.getenv(_SM120_NVFP4_K3_GRAPH_GATE_ENV) == "1"
        profiling_free_before = 0
        profiling_free_with_graphs = 0
        profiling_allocated_before = 0
        profiling_peak_allocated = 0
        if sm120_graph_campaign:
            torch.accelerator.synchronize()
            torch.accelerator.empty_cache()
            profiling_free_before = torch.accelerator.get_memory_info()[0]
            profiling_allocated_before = torch.cuda.memory_allocated(self.device)
        ...
        try:
            ...
            with (
                ...
                if sm120_graph_campaign:
                    torch.accelerator.synchronize()
                    profiling_free_with_graphs = torch.accelerator.get_memory_info()[0]
                    profiling_peak_allocated = torch.cuda.max_memory_allocated(
                        self.device
                    )
        finally:
        ...
        if sm120_graph_campaign:
            torch.accelerator.synchronize()
            torch.accelerator.empty_cache()
            profiling_free_after_cleanup = torch.accelerator.get_memory_info()[0]
            self._sm120_graph_profile_memory = {
                "throwaway_retained_graph_bytes": max(
                    profiling_free_before - profiling_free_with_graphs, 0
                ),
                "profiling_capture_peak_allocated_bytes": max(
                    profiling_peak_allocated - profiling_allocated_before, 0
                ),
                "profiling_free_before_bytes": profiling_free_before,
                "profiling_free_with_graphs_bytes": profiling_free_with_graphs,
                "profiling_free_after_cleanup_bytes": profiling_free_after_cleanup,
            }
    ...
    def capture_model(self) -> int:
        ...
        sm120_graph_campaign = os.getenv(_SM120_NVFP4_K3_GRAPH_GATE_ENV) == "1"
        capture_allocated_before = 0
        if sm120_graph_campaign:
            capture_allocated_before = torch.cuda.memory_allocated(self.device)
            torch.cuda.reset_peak_memory_stats(self.device)
        ...
        with self._freeze_gc(), graph_capture(device=self.device):
            ...
            if os.getenv("VLLM_SM120_MTP_DECODE_GRAPH") == "1":
                from vllm.v1.spec_decode.sm120_mtp_decode_graph import (
                    SM120MTPDecodeGraph,
                )

                self.drafter._sm120_mtp_decode_graph = SM120MTPDecodeGraph(self.drafter)
                self.drafter._sm120_mtp_decode_graph.capture()
        ...
        if sm120_graph_campaign:
            physical_free_after_capture, physical_total = (
                torch.accelerator.get_memory_info()
            )
            self._sm120_graph_actual_memory = {
                "actual_retained_graph_bytes": max(
                    start_free_gpu_memory - physical_free_after_capture, 0
                ),
                "actual_capture_peak_allocated_bytes": max(
                    torch.cuda.max_memory_allocated(self.device)
                    - capture_allocated_before,
                    0,
                ),
                "physical_free_after_capture_bytes": physical_free_after_capture,
    ...
    def _check_and_update_cudagraph_mode(
        ...
        backend_capabilities: dict[str, AttentionCGSupport] = {}
        ...
        for attn_backend_set, kv_cache_group in zip(
            ...
            for attn_backend in attn_backend_set:
                ...
                backend_name = attn_backend.__name__
                previous_support = backend_capabilities.get(backend_name)
                if (
                    previous_support is None
                    or cg_support.value < previous_support.value
                ):
                    backend_capabilities[backend_name] = cg_support
        ...
        if self.speculative_config and (
            ...
            if (
                os.getenv(_SM120_NVFP4_K3_GRAPH_GATE_ENV) == "1"
                and self.speculative_config.method == "mtp"
            ):
                # The campaign captures only the target K+1 verifier. The
                # MTP proposer inherits Eagle's graph policy, so disable its
                # dispatcher explicitly instead of letting the target
                # FULL_DECODE_ONLY mode create drafter graph shapes.
        ...
        if os.getenv(_SM120_NVFP4_K3_GRAPH_GATE_ENV) == "1" and not is_profiling:
            drafter_dispatcher = getattr(
                getattr(self, "drafter", None), "cudagraph_dispatcher", None
            )
            record = {
                "rank": get_tensor_model_parallel_rank(),
                "backend_capabilities": {
                    name: support.name
                    for name, support in sorted(backend_capabilities.items())
                },
                "aggregate": min_cg_support.name,
                "use_v2_model_runner": False,
                "compilation_mode": self.compilation_config.mode.name,
                "requested_mode": requested_mode.name,
                "resolved_mode": cudagraph_mode.name,
                "uniform_decode_query_len": self.uniform_decode_query_len,
                "capture_sizes": list(
                ...
                "target_descriptors": _cudagraph_descriptors(self.cudagraph_dispatcher),
                "drafter_descriptors": (
                    _cudagraph_descriptors(drafter_dispatcher)
                    if drafter_dispatcher is not None
                    else []
                ),
            }
            logger.info(
                "%s%s",
                _SM120_NVFP4_K3_GRAPH_RECORD_PREFIX,
                json.dumps(record, separators=(",", ":"), sort_keys=True),
            )
            _validate_sm120_nvfp4_k3_graph_resolution(record)
